...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.promocion-productos.store') }}" method="POST" class="bg-white/5 backdrop-blur-xl rounded-2xl p-8 border border-white/5 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej: Desayuno Especial" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
        </div>
