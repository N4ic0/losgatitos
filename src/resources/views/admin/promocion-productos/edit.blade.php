...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.promocion-productos.update', $promocionProducto) }}" method="POST" class="bg-white/5 backdrop-blur-xl rounded-2xl p-8 border border-white/5 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <select name="promocion_id" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
                    ...
                    <option value="{{ $p->id }}" {{ old('promocion_id', $promocionProducto->promocion_id) == $p->id ? 'selected' : '' }} class="bg-gray-900">{{ $p->titulo }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="producto_id" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
                    ...
                    <option value="{{ $prod->id }}" {{ old('producto_id', $promocionProducto->producto_id) == $prod->id ? 'selected' : '' }} class="bg-gray-900">{{ $prod->nombre }} ({{ $prod->categoria }}) - ${{ number_format($prod->precio, 0, '', '.') }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-gray-300 text-sm font-medium mb-2">Cantidad</label>
                <input type="number" name="cantidad" value="{{ old('cantidad', $promocionProducto->cantidad) }}" required min="1" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
            </div>
        </div>
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" class="bg-[#D4AF37] hover:bg-[#C49A2C] text-black font-semibold px-6 py-3 rounded-xl transition-all">Actualizar Paquete</button>
        </div>
    </form>
</div>
@endsection
