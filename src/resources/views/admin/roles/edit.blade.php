...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/5 p-8 space-y-6">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-gray-300 text-sm font-medium mb-2">Nombre del Rol</label>
                <input type="text" name="name" value="{{ old('name', $role->name) }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
                @error('name') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-gray-300 text-sm font-medium mb-2">Identificador (slug)</label>
                <input type="text" name="slug" value="{{ old('slug', $role->slug) }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none">
        ...
        <div>
            ...
            <textarea name="description" rows="2" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none resize-none">{{ old('description', $role->description) }}</textarea>
        </div>
                ...
                <div class="bg-white/5 rounded-xl p-4">
                    ...
                    <div id="group-{{ Str::slug($group) }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 ml-6">
                        ...
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="permissions[]" value="{{ $key }}" {{ in_array($key, $rolePermissions) ? 'checked' : '' }} class="perm-checkbox group-{{ Str::slug($group) }} w-4 h-4 rounded bg-white/5 border-white/10 text-[#D4AF37] focus:ring-[#D4AF37]">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" class="bg-[#D4AF37] hover:bg-[#C49A2C] text-black font-semibold px-6 py-3 rounded-xl transition-all">Actualizar Rol</button>
        </div>
    </form>
</div>
@endsection
